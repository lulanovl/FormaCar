...
export default function Calendar({ isActive, refreshKey }) {
  const today = new Date().toISOString().split('T')[0];
  const [selectedDate, setSelectedDate] = useState(today);
  const [orders, setOrders] = useState([]);
  const [loading, setLoading] = useState(true);

  // derive year/month from selectedDate so CalendarPicker drives navigation too
    ...
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1px', background: 'var(--border)' }}>
        ...
        <div className="crm-box">
          <div className="crm-box-title">Выберите дату</div>
          <CalendarPicker
            value={selectedDate}
        ...
        <div className="crm-box">
          <div className="crm-box-title">
            {loading ? 'Загрузка...' : `Слоты · ${selDateLabel}`}
          </div>
              ...
              {SLOTS.map(time => {
                ...
                return (
                  <div key={time} className={`slot-item slot-item-multi ${free === 0 ? 'full' : ''}`}>
                    ...
                    {slotOrders.map(o => (
                      <div key={o.id} className="slot-order-card">
